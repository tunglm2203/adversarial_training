    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            with autocast():
                output = model(x_adv)
                loss_ce = F.cross_entropy(output, y)
            scaled_loss = scaler.scale(loss_ce)
            grad = torch.autograd.grad(scaled_loss, [x_adv],
                                    retain_graph=False, create_graph=False)[0]
...
def trades_loss(model,
    ...
    if distance == 'l_inf':
        for _ in range(perturb_steps):
            ...
            scaled_loss = scaler.scale(loss_kl)
            grad = torch.autograd.grad(scaled_loss, [x_adv],
                                    retain_graph=False, create_graph=False)[0]
...
def eval_robust(model, device, test_loader,epoch,optimizer, epsilon, test_bn="pgd", perturb_steps=20, eval_scaler=None):
    model.eval()
    test_loss = 0
    correct = 0
    # with torch.no_grad():
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        data_adv = generate_adv(model, data, target, optimizer, step_size=epsilon/4, epsilon=epsilon, perturb_steps=perturb_steps, bn_name=test_bn, eval_scaler=eval_scaler)
...
def generate_adv(model,
    ...
    for _ in range(perturb_steps):
        ...
        if eval_scaler:
            ...
            scaled_loss = eval_scaler.scale(cost)
            grad = torch.autograd.grad(scaled_loss, [adv_images],
                                    retain_graph=False, create_graph=False)[0]
...
def eval_test(model, device, test_loader, epoch, test_bn="pgd", eval_scaler=None):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
