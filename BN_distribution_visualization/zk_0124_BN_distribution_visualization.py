...
def main():
    ...
    FORWARD_DATA = ["clean", "ae_adv", "ae_clean"]
    ...
    target_layer = [("layer3.1.bn2.bn_list.0.running_mean"), 
                    ("layer3.0.bn1.bn_list.0.running_mean"),

                    ("layer3.1.bn2.bn_list.0.running_var"),
                    ("layer3.0.bn1.bn_list.0.running_var"),

                    ("layer3.1.bn2.bn_list.1.running_mean"), 
                    ("layer3.0.bn1.bn_list.1.running_mean"),

                    ("layer3.1.bn2.bn_list.1.running_var"),
                    ("layer3.0.bn1.bn_list.1.running_var"),
    ...
    target_layer = [("layer3.1.bn2.bn_list.0.weight"), 
                    ("layer3.1.bn2.bn_list.0.bias"), 
                    ("layer3.1.bn2.bn_list.1.weight"), 
                    ("layer3.1.bn2.bn_list.1.bias"), 
                    ]
    ...
    figure_data = {
        r"affine$_{adv}$": saving_gamma_beta[0],
        r"affine$_{clean}$": saving_gamma_beta[2],
    }
    plt.figure(figsize=(15, 5))    
    plt.subplot(2,1,1)
    draw_figure(figure_data, f"BN_distribution_visualization/figure/gamma_value.pdf", shuffle_index=shuffle_index, ylabel=r"$\gamma$", legend=True)

    figure_data = {
        r"affine$_{adv}$": saving_gamma_beta[1],
        r"affine$_{clean}$": saving_gamma_beta[3],
    }
    plt.subplot(2,1,2)

    draw_figure(figure_data, f"BN_distribution_visualization/figure/beta_value.pdf", shuffle_index=shuffle_index, ylabel=r"$\beta$", legend=True)
    # plt.tight_layout()

    plt.savefig(f"BN_distribution_visualization/figure/beta_gamma_.pdf", bbox_inches="tight")
    ...
    clean_forward_stat_bn_pgd = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "clean", args.num_steps, args.epsilon, args.step_size, "pgd", target_layer)
    ae_adv_forward_stat_bn_pgd = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "ae_adv", args.num_steps, args.epsilon, args.step_size, "pgd", target_layer)
    ae_clean_forward_stat_bn_pgd = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "ae_clean", args.num_steps, args.epsilon, args.step_size, "pgd", target_layer)

    target_layer = [("layer3.1.bn2.bn_list.1.running_mean"), 
                    ("layer3.0.bn1.bn_list.1.running_mean"),
    ...
    clean_forward_stat_bn_normal = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "clean", args.num_steps, args.epsilon, args.step_size, "normal", target_layer)
    ae_adv_forward_stat_bn_normal = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "ae_adv", args.num_steps, args.epsilon, args.step_size, "normal", target_layer)
    ae_clean_forward_stat_bn_normal = get_forward_data_stat(multi_bn_resnet18(norm_layer=norm_layer, bn_names=["pgd", "normal"], state_dict_dir=args.resume_dir).cuda(), test_loader, "ae_clean", args.num_steps, args.epsilon, args.step_size, "normal", target_layer)
    ...
    # figure_data = {
    #     r"running$_{adv}$,affine$_{adv}$":saving_stat[0],
    #     r"CE,static$_{clean}$,affine$_{adv}$":clean_forward_stat_bn_pgd[0],
    #     r"AE,static$_{adv}$,affine$_{adv}$":ae_adv_forward_stat_bn_pgd[0],
    #     r"running$_{clean}$,affine$_{clean}$":saving_stat[4],
    #     r"CE,static$_{clean}$,affine$_{clean}$":clean_forward_stat_bn_normal[0],
    #     r"AE,static$_{adv}$,affine$_{clean}$":ae_adv_forward_stat_bn_normal[0],
    # }
    figure_data = {
        r"Running$_{adv}$":saving_stat[0],
        r"Static$_{adv}^{clean}$":clean_forward_stat_bn_pgd[0],
        r"Static$_{adv}^{adv}$":ae_adv_forward_stat_bn_pgd[0],
        r"Running$_{clean}$":saving_stat[4],
        r"Static$_{clean}^{clean}$":clean_forward_stat_bn_normal[0],
        r"Static$_{clean}^{adv}$":ae_adv_forward_stat_bn_normal[0],
    }
    ...
    plt.subplot(2,1,1)
    draw_figure(figure_data, f"BN_distribution_visualization/figure/mean_all.pdf", shuffle_index=shuffle_index, ylabel="Mean", legend=True)
    ...
    draw_figure(figure_data, f"BN_distribution_visualization/figure/var_all.pdf", shuffle_index=shuffle_index, ylabel="Variance")
    # plt.tight_layout()

    plt.savefig(f"BN_distribution_visualization/figure/mean_std_.pdf", bbox_inches="tight")
...
def get_forward_data_stat(model_for_test, test_loader, forward_data, num_steps, epsilon, step_size, bn_name, target_layer):
    """
...
def draw_figure(draw_data, file_name, shuffle_index=None, ylabel=None, legend=None):
    """
