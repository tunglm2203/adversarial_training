...
def eval_robust(model, device, test_loader,epoch,optimizer, epsilon, test_bn="pgd",  perturb_steps=20, logger=None):
    ...
    for batch_id, (data, target) in enumerate(test_loader):
        ...
        # import ipdb; ipdb.set_trace()
        data_adv = generate_adv(model, data, target, optimizer, step_size=epsilon/4, epsilon=epsilon, perturb_steps=perturb_steps)
        # data_adv = data
        ...
        if logger:
            logger.info('batch id: {}.Test: (robust) loss: {:.4f}, Robust accuracy: {}/{} ({:.0f}%)'.format(
