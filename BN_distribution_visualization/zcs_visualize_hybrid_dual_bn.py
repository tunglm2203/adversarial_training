...
def main():
    ...
    plt.savefig(f"BN_distribution_visualization/zcs_figure/zcs_hybrid_dual_bn_beta_gamma_.png", bbox_inches="tight")
    ...
    figure_data = {
        r"NS$_{adv}^{adv}$ (NS$_{adv}$)":saving_stat[0],
        r"NS$_{clean}^{adv}$":clean_forward_stat_bn_pgd[0],
        r"NS$_{clean}^{clean}$ (NS$_{clean}$)":saving_stat[4],
    ...
    figure_data = {
        r"NS$_{adv}^{adv}$":saving_stat[2],
        r"NS$_{clean}^{adv}$":clean_forward_stat_bn_pgd[2],
        r"NS$_{clean}^{clean}}$":saving_stat[6],
        r"NS$_{adv}^{clean}$":ae_adv_forward_stat_bn_normal[2],
    }
    ...
    plt.savefig(f"BN_distribution_visualization/zcs_figure/zcs_hybrid_dual_bn_mean_std_.png", bbox_inches="tight")
...
def get_forward_data_stat(model_for_test, test_loader, forward_data, num_steps, epsilon, step_size, bn_name, target_layer):
    ...
    for layers in target_layer:
        if "running_var" in layers:
           target_stat.append( torch.sqrt(model_state_dict[layers].data))
        else:
