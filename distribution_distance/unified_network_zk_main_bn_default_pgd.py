...
def main():
    ...
    parser.add_argument('--wandb_entity', type=str, default="kaistssl")
    ...
    from models.resnet_multi_bn_default_pgd import CrossTrainingBN_clean, CrossTrainingBN_adv, CrossTrainingBN_all, Disentangling_LP, BN2dStrongerAT, Disentangling_StatP, BN2dStrongerATMomentum1
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model_for_measure.load_state_dict(copy.deepcopy(model.state_dict()))
        # get the stat for clean image
        model_for_measure.train()
        for batch_id, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
            data_adv = generate_adv(model_for_measure, data, target, optimizer, step_size=args.epsilon/4, epsilon=args.epsilon, perturb_steps=10)
            model_for_measure.train()
        ...
        for (adv_bn_stat_key, adv_bn_stat_value), (clean_bn_stat_key, clean_bn_stat_value) in zip(adv_stat.items(), clean_stat.items()):
            if "bn_list.0.running_mean" in adv_bn_stat_key:
                ...
                dis_measure_saver[adv_bn_stat_key] = adv_clean_wass_dis
            if "bn_list.0.running_var" in adv_bn_stat_key:
                adv_clean_wass_dis = wass_dis(adv_bn_stat_value.cpu().numpy(), clean_bn_stat_value.cpu().numpy())
                dis_measure_saver[adv_bn_stat_key] = adv_clean_wass_dis

        # import ipdb; ipdb.set_trace()
        dis_measure_saver_path = os.path.join(args.out_dir, f'dis_measure_{epoch}.pth')
        os.makedirs(os.path.dirname(dis_measure_saver_path), exist_ok=True)
        torch.save(dis_measure_saver, dis_measure_saver_path)

        # torch.save(dis_measure_saver_path, dis_measure_saver)


        if args.training_method == "Madry_mixture_bn" or args.training_method == "Madry_dual_bn":
            ########################################
            # Clean mean and std + PGD gama and beta
            model_state_dict = model.state_dict()

            # copy gama and beta
            for layer, weight  in model_state_dict.items():
                if "bn_list.0.weight" in layer:
                    model_state_dict[layer.replace("bn_list.0.weight", "bn_list.1.weight")].data = model_state_dict[layer].data
                if "bn_list.0.bias" in layer:
                    model_state_dict[layer.replace("bn_list.0.bias", "bn_list.1.bias")].data = model_state_dict[layer].data
            ...
            test_loss, test_accuracy = eval_robust(model_for_test, device, test_loader, epoch, optimizer, args.epsilon, test_bn="normal", perturb_steps=10)
            metrics["robust_loss-cross_clean"] = test_loss
            metrics["robust_acc-cross_clean"] = test_accuracy * 100

            test_loss, test_accuracy = eval_test(model_for_test, device, test_loader, epoch, test_bn="normal")
            metrics["clean_loss-cross_clean"] = test_loss
            metrics["clean_acc-cross_clean"] = test_accuracy * 100

            del model_for_test
            # model.load_state_dict(model_state_dict)

            ########################################
            # PGD mean and std + Clean gama and beta
            model_state_dict = model.state_dict()
            ...
            test_loss, test_accuracy = eval_robust(model_for_test, device, test_loader, epoch, optimizer, args.epsilon, test_bn="pgd", perturb_steps=10)
            metrics["robust_loss-cross_pgd"] = test_loss
            metrics["robust_acc-cross_pgd"] = test_accuracy * 100

            test_loss, test_accuracy = eval_test(model_for_test, device, test_loader, epoch, test_bn="pgd")
            metrics["clean_loss-cross_pgd"] = test_loss
            metrics["clean_acc-cross_pgd"] = test_accuracy * 100
