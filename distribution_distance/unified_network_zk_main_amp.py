...
from training_method_amp import train, eval_robust, eval_test
...
def main():
    ...
    BN_LAYER_LIST = ["vanilla", "oct_adv", "oct_clean", "oct_all"]
    ...
    SUPPORT_METHOD = ["trades_vanilla", "trades_dual_bn", "trades_cat", "Madry_loss", "Madry_cat_loss"]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss = train(args, model, device, train_loader, optimizer, epoch, scaler)
        ...
        eval_scaler = GradScaler()
        # if args.model in ["resnet18"]:
        if not args.dual_bn:

            # eval_train(model, device, train_loader)

            test_loss, test_accuracy = eval_robust(model, device, test_loader, epoch, optimizer, args.epsilon, test_bn="normal", perturb_steps=10, eval_scaler=eval_scaler)
            ...
            test_loss, test_accuracy = eval_test(model, device, test_loader, epoch, test_bn="normal", eval_scaler=eval_scaler)
            metrics["clean_loss_bn_normal"] = test_loss
            metrics["clean_acc_bn_normal"] = test_accuracy * 100

        else:
            test_loss, test_accuracy = eval_robust(model, device, test_loader, epoch, optimizer, args.epsilon, test_bn="pgd", perturb_steps=10, eval_scaler=eval_scaler)
            ...
            test_loss, test_accuracy = eval_test(model, device, test_loader, epoch, test_bn="pgd", eval_scaler=eval_scaler)
